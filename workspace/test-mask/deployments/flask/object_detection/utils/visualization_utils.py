...
def draw_mask_on_image_array(image, mask, color='red', alpha=0.4):
  ...
  """
    ...
    pil_mask = Image.fromarray(np.uint8(255.0 * alpha *
                                        (mask > 0))).convert('L')
...
def draw_keypoints_on_image(image,
  ...
  """
    ...
    for keypoint_x, keypoint_y, valid in zip(keypoints_x, keypoints_y,
                                             valid_kpt):
        if valid:
    ...
    if keypoint_edges is not None:
        for keypoint_start, keypoint_end in keypoint_edges:
            if (keypoint_start < 0 or keypoint_start >= len(keypoints)
                    or keypoint_end < 0 or keypoint_end >= len(keypoints)):
                continue
            ...
            draw.line(edge_coordinates,
                      fill=keypoint_edge_color,
                      width=keypoint_edge_width)
